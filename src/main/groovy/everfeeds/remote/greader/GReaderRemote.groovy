...
class GReaderRemote extends Remote {
  ...
  List<TagD> getActualizedTags(AccessD access) {
    ...
    RemoteUtils.actualizeTags(access, jsonTags, {it.id}, {it.id.substring(it.id.lastIndexOf("/") + 1)})
  }

  @Override
  List<CategoryD> getActualizedCategories(AccessD access) {
    OAuthAccess oAuthAccess = new OAuthAccess(access)
    List jsonCategories = raw.getJson(oAuthAccess, GReaderRawUrl.SUBSCRIPTION_LIST)?.subscriptions
    RemoteUtils.actualizeCategories(access, jsonCategories, {it.id}, {it.title})
  }
  ...
  List<EntryD> pull(FilterD filterD, int max, int offset) {
    ...
    def result = raw.getJson(oAuthAccess, url, urlSuffix)
